...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .version-link:hover, .version-link:focus-visible { text-decoration: underline; }
    main {
      display: grid;
    ...
    .brief-grid {
      display: grid;
      grid-template-columns: 1.25fr 1fr 76px;
      gap: 8px;
    }
    .icon-button {
      width: 34px;
    ...
    .candidate-table { min-width: 760px; }
    .candidate-table td:nth-child(2), .candidate-table th:nth-child(2) { text-align: right; }
    .candidate-table td:last-child, .candidate-table th:last-child { width: 38px; text-align: right; }
    .candidate-table tr { cursor: default; }
    .candidate-table .candidate-row { cursor: pointer; }
    .evidence-trigger {
      width: auto;
      border: 0;
      padding: 0;
    ...
    .evidence-detail-row td {
      padding: 0 8px 10px;
      background: var(--bg);
    }
    .evidence-detail-row[hidden] { display: none; }
    .evidence-detail {
      padding: 8px 10px;
      color: var(--muted);
      font-size: 11px;
    ...
    .evidence-detail div + div { margin-top: 4px; }
    .status-label { display: block; font-weight: 600; }
    .status-shortlisted { color: var(--accent); }
    .status-eligible { color: var(--warning); }
    .status-excluded { color: var(--risk); }
    .status-reason {
      display: block;
      min-width: 150px;
      margin-top: 3px;
    ...
    @media (max-width: 520px) {
      ...
      .version-link { display: inline-block; margin-top: 7px; }
      .inputs, .button-row, .brief-grid, .metrics { grid-template-columns: 1fr; }
      .builder-actions { grid-template-columns: 1fr auto; }
    }
  </style>
</head>
<body>
  <header>
    <div>
      ...
      <p class="subtitle">Evaluate, compare, and shortlist outdoor advertising candidate locations</p>
    </div>
    <a class="version-link" href="dashboard.html">Open V1 Location Assistant</a>
  </header>
  <main>
    ...
    <aside>
      ...
      <section>
        <h2>Campaign Shortlist Builder</h2>
        <p class="assistant-scope">Add candidate locations, define the campaign criteria, and identify which sites deserve field validation.</p>
        <div class="builder-actions">
          <button id="saveCandidateBtn">Add Current Location</button>
          <button class="icon-button" id="clearCandidatesBtn" title="Clear all candidate locations" aria-label="Clear all candidate locations">&times;</button>
        </div>
        <div class="brief-grid">
          <label>Primary objective<select id="campaignObjective">
            <option value="exposure">Maximum weekly exposure</option>
            <option value="tourism">Tourism awareness</option>
            <option value="retail">Retail / high-street</option>
            <option value="transit">Transit / commuter</option>
          </select></label>
          <label>Evidence policy<select id="evidencePolicy">
            <option value="balanced">Balanced</option>
            <option value="strict">Strict</option>
            <option value="exploratory">Exploratory</option>
          </select></label>
          <label>Maximum shortlist size<input id="shortlistSize" type="number" min="1" max="8" value="3"></label>
        </div>
        <p class="note" id="campaignRule"></p>
        <p class="note" id="candidateWorkspaceNote"></p>
        <div id="demoAction" class="demo-action"><button class="text-action" id="sampleCandidatesBtn">Try with demo locations</button></div>
        <div class="table-wrap"><table class="candidate-table">
          <thead><tr><th>Candidate</th><th>Weekly estimate</th><th>80% interval</th><th>Campaign fit</th><th>Evidence</th><th>Status</th><th></th></tr></thead>
          <tbody id="savedCandidates"></tbody>
        </table></div>
      </section>
      <section>
        <h2>Ask the Shortlist Agent</h2>
        <p class="assistant-scope">Ask why a site was selected or excluded, compare the leading candidates, or change the shortlist criteria.</p>
        <div id="suggestions" class="suggestions" aria-label="Suggested questions"></div>
        <textarea id="askInput" placeholder="For example: Why was this location excluded?"></textarea>
  ...
  <script>
    ...
    const candidateStorageKey = 'amsterdam-location-agent-v2-candidates';
    ...
    function reliabilityFlagDetails(row) {
      const flags = [];
      if (row.comparableError > 0.75) {
        flags.push({
          label: 'similar-anchor validation error',
          detail: `${Math.round(row.comparableError * 100)}% average held-out error exceeds the 75% caution threshold.`
        });
      }
      if (row.variability > 0.45) {
        flags.push({
          label: 'historical variability',
          detail: `${Math.round(row.variability * 100)}% weekly spread exceeds the 45% caution threshold.`
        });
      }
      if (row.outsideFeatureCount > 0) {
        flags.push({
          label: 'inputs outside the training range',
          detail: `${row.outsideFeatureCount} of ${model.features.length} model inputs fall outside values observed in training.`
        });
      }
      if (row.supportCount === 0 || row.jointSimilarityPct < 25) {
        const supportText = row.supportCount === 0
          ? 'No anchor is within the typical feature-match threshold'
          : `${row.supportCount} anchor${row.supportCount === 1 ? '' : 's'} fall within the typical feature-match threshold`;
        flags.push({
          label: 'weak joint feature similarity',
          detail: `${supportText}; joint similarity is ${Math.round(row.jointSimilarityPct)}%, with 25% used as the caution threshold.`
        });
      }
      return flags;
    }
    function reliabilityFlags(row) {
      return reliabilityFlagDetails(row).map(item => item.label);
    }
    function candidateKey(row) {
      return `${Number(row.lat).toFixed(6)},${Number(row.lon).toFixed(6)}`;
    }
    ...
    function persistCandidates() {
      try {
        localStorage.setItem(candidateStorageKey, JSON.stringify(savedCandidates));
      } catch (error) {
        // The workspace still works for this session when browser storage is unavailable.
      }
    }
    function restoreCandidates() {
      try {
        const restored = JSON.parse(localStorage.getItem(candidateStorageKey) || '[]');
        if (Array.isArray(restored)) savedCandidates = restored.slice(0, 12);
      } catch (error) {
        savedCandidates = [];
      }
    }
    function evidenceSummary(row) {
      const flags = reliabilityFlagDetails(row);
      return flags.length ? `${flags.length} caution${flags.length === 1 ? '' : 's'}` : 'No cautions triggered';
    }
    function evidenceDetailHtml(row) {
      const flags = reliabilityFlagDetails(row);
      if (!flags.length) {
        return '<div><strong>No cautions triggered.</strong> None of the four local evidence checks crosses its caution threshold. This supports screening use but does not guarantee prediction accuracy.</div>';
      }
      return flags.map(item => `<div><strong>${escapeHtml(item.label.charAt(0).toUpperCase() + item.label.slice(1))}:</strong> ${escapeHtml(item.detail)}</div>`).join('');
    }
    function renderCandidateWorkspace() {
      const body = document.getElementById('savedCandidates');
      const demoAction = document.getElementById('demoAction');
      demoAction.hidden = savedCandidates.length > 0;
      if (!savedCandidates.length) {
        lastAgentRun = null;
        document.getElementById('candidateWorkspaceNote').textContent = 'No candidate locations yet. Evaluate a location above, then add it here.';
        body.innerHTML = '';
        return;
      }
      lastAgentRun = buildShortlist(campaignBrief());
      const selectedKeys = new Set(lastAgentRun.selected.map(candidateKey));
      const eligibleKeys = new Set(lastAgentRun.eligible.map(candidateKey));
      const exclusionLookup = new Map(lastAgentRun.excluded.map(item => [candidateKey(item.candidate), item.reasons]));
      const ordered = rankCandidates(savedCandidates);
      document.getElementById('candidateWorkspaceNote').textContent = `${savedCandidates.length} candidate${savedCandidates.length === 1 ? '' : 's'} ranked by predicted weekly exposure. ${lastAgentRun.selected.length} of up to ${lastAgentRun.brief.shortlistSize} shortlisted for field validation. ${lastAgentRun.eligible.length} passed the current campaign criteria; ${lastAgentRun.excluded.length} excluded.`;
      body.innerHTML = ordered.map((row, index) => {
        const key = candidateKey(row);
        const isSelected = selectedKeys.has(key);
        const isEligible = eligibleKeys.has(key);
        const status = isSelected ? 'Shortlisted' : isEligible ? 'Eligible' : 'Excluded';
        const statusClass = isSelected ? 'status-shortlisted' : isEligible ? 'status-eligible' : 'status-excluded';
        const reason = isSelected
          ? 'Top eligible site for field validation.'
          : isEligible
            ? 'Passed the criteria but falls below the shortlist cutoff.'
            : (exclusionLookup.get(key) || ['Does not meet the current criteria.']).join('; ');
        const evidenceLabel = evidenceSummary(row);
        return `
        <tr class="candidate-row" data-key="${candidateKey(row)}">
          <td><strong>${index + 1}. ${escapeHtml(row.name)}</strong></td>
          <td>${fmt(row.predicted)}</td>
          <td>${fmt(row.low)}-${fmt(row.high)}</td>
          <td>${escapeHtml(campaignFitText(row, lastAgentRun.brief.objective))}</td>
          <td><button class="evidence-trigger" data-key="${candidateKey(row)}" aria-expanded="false">${escapeHtml(evidenceLabel)}</button></td>
          <td><span class="status-label ${statusClass}">${status}</span><span class="status-reason">${escapeHtml(reason)}</span></td>
          <td><button class="icon-button remove-candidate" data-key="${candidateKey(row)}" title="Remove ${escapeHtml(row.name)}" aria-label="Remove ${escapeHtml(row.name)}">&times;</button></td>
        </tr>
        <tr class="evidence-detail-row" data-key="${candidateKey(row)}" hidden>
          <td colspan="7"><div class="evidence-detail">${evidenceDetailHtml(row)}</div></td>
        </tr>
      `}).join('');
      body.querySelectorAll('.candidate-row').forEach(rowElement => {
        rowElement.addEventListener('click', event => {
          if (event.target.closest('.remove-candidate')) return;
          const candidate = savedCandidates.find(item => candidateKey(item) === rowElement.dataset.key);
          if (!candidate) return;
          current = candidateSnapshot(candidate);
          document.getElementById('candidateName').value = current.name;
          document.getElementById('latInput').value = Number(current.lat).toFixed(6);
          document.getElementById('lonInput').value = Number(current.lon).toFixed(6);
          renderResult(current);
          placeCustomMarker(current);
      ...
      body.querySelectorAll('.remove-candidate').forEach(button => {
        button.addEventListener('click', () => {
          savedCandidates = savedCandidates.filter(item => candidateKey(item) !== button.dataset.key);
          lastAgentRun = null;
          persistCandidates();
          renderCandidateWorkspace();
          if (current) renderSuggestions(current);
        });
      });
      body.querySelectorAll('.evidence-trigger').forEach(button => {
        button.addEventListener('click', event => {
          event.stopPropagation();
          const detailRow = Array.from(body.querySelectorAll('.evidence-detail-row')).find(item => item.dataset.key === button.dataset.key);
          if (!detailRow) return;
    ...
    function saveCurrentCandidate() {
      ...
      const existing = savedCandidates.findIndex(item => candidateKey(item) === key);
      if (existing >= 0) savedCandidates[existing] = candidateSnapshot(current);
      else if (savedCandidates.length < 12) savedCandidates.push(candidateSnapshot(current));
      lastAgentRun = null;
      persistCandidates();
      renderCandidateWorkspace();
      return existing >= 0 ? `Updated ${current.name} in the candidate workspace.` : `Saved ${current.name} to the candidate workspace.`;
    }
    function loadSampleCandidates() {
      const positions = [0, 0.25, 0.5, 0.75, 1].map(fraction => Math.round((anchors.length - 1) * fraction));
      const unique = [...new Set(positions)].map(index => anchors[index]);
      const selected = current ? candidateSnapshot(current) : null;
      for (const anchor of unique) {
        const candidate = evaluate(anchor.lat, anchor.lon, anchor.name);
        const key = candidateKey(candidate);
        const existing = savedCandidates.findIndex(item => candidateKey(item) === key);
        if (existing >= 0) savedCandidates[existing] = candidateSnapshot(candidate);
        else if (savedCandidates.length < 12) savedCandidates.push(candidateSnapshot(candidate));
      }
      lastAgentRun = null;
      ...
      return `Loaded ${unique.length} training-anchor examples spanning different exposure levels and location contexts.`;
    }
    const objectiveLabels = {
      exposure: 'maximum weekly exposure',
      tourism: 'tourism awareness',
      retail: 'retail / high-street activity',
      transit: 'transit / commuter activity'
    };
    function campaignBrief() {
      return {
        objective: document.getElementById('campaignObjective').value,
        evidencePolicy: document.getElementById('evidencePolicy').value,
        shortlistSize: Math.max(1, Math.min(8, Number(document.getElementById('shortlistSize').value) || 3))
      };
    }
    function campaignRuleText(brief = campaignBrief()) {
      const contextRule = brief.objective === 'exposure'
        ? 'No context filter; eligible sites are ranked by predicted weekly exposure.'
        : `${objectiveLabels[brief.objective]} requires the matching context percentile to be at least 60.`;
      const evidenceRule = brief.evidencePolicy === 'strict'
        ? 'Strict evidence requires zero out-of-range inputs, comparable error <=75%, stability <=45%, joint similarity >=25%, and at least one typical feature match.'
        : brief.evidencePolicy === 'balanced'
          ? 'Balanced evidence allows at most one out-of-range input, comparable error <=150%, and joint similarity >=10%.'
          : 'Exploratory evidence does not exclude sites, but caution signals remain visible.';
      return `${contextRule} ${evidenceRule}`;
    }
    function renderCampaignRule() {
      document.getElementById('campaignRule').textContent = campaignRuleText();
    }
    function objectiveReasons(row, brief) {
      if (brief.objective === 'tourism' && row.tourismPct < 60) return [`tourism context is percentile ${Math.round(row.tourismPct)}, below 60`];
      if (brief.objective === 'retail' && row.retailPct < 60) return [`retail context is percentile ${Math.round(row.retailPct)}, below 60`];
      if (brief.objective === 'transit' && row.transitPct < 60) return [`transit context is percentile ${Math.round(row.transitPct)}, below 60`];
      return [];
    }
    function evidenceReasons(row, brief) {
      const reasons = [];
      if (brief.evidencePolicy === 'strict') {
        if (row.outsideFeatureCount > 0) reasons.push(`${row.outsideFeatureCount} inputs outside training range`);
        if (row.comparableError > 0.75) reasons.push(`${Math.round(row.comparableError * 100)}% similar-anchor held-out error exceeds 75%`);
        if (row.variability > 0.45) reasons.push(`${Math.round(row.variability * 100)}% historical spread exceeds 45%`);
        if (row.jointSimilarityPct < 25) reasons.push(`${Math.round(row.jointSimilarityPct)}% joint similarity is below 25%`);
        if (row.supportCount === 0) reasons.push('no anchor falls within the typical feature-match threshold');
      } else if (brief.evidencePolicy === 'balanced') {
        if (row.outsideFeatureCount > 1) reasons.push(`${row.outsideFeatureCount} inputs outside training range`);
        if (row.comparableError > 1.5) reasons.push(`${Math.round(row.comparableError * 100)}% similar-anchor held-out error exceeds 150%`);
        if (row.jointSimilarityPct < 10) reasons.push(`${Math.round(row.jointSimilarityPct)}% joint similarity is below 10%`);
      }
      return reasons;
    }
    function filterCandidates(brief) {
      const eligible = [];
      const excluded = [];
      for (const row of savedCandidates) {
        const reasons = [...objectiveReasons(row, brief), ...evidenceReasons(row, brief)];
        if (reasons.length) excluded.push({candidate: row, reasons});
        else eligible.push(row);
      }
      return {eligible, excluded};
    }
    function rankCandidates(rows) {
      return [...rows].sort((left, right) => right.predicted - left.predicted);
    }
    function buildShortlist(brief = campaignBrief()) {
      const filtered = filterCandidates(brief);
      const ranked = rankCandidates(filtered.eligible);
      const selected = ranked.slice(0, brief.shortlistSize);
      return {...filtered, ranked, selected, brief};
    }
    function renderSuggestions(row) {
      const brief = campaignBrief();
      const isSaved = savedCandidates.some(item => candidateKey(item) === candidateKey(row));
      const flags = reliabilityFlagDetails(row);
      const selectedKeys = new Set((lastAgentRun?.selected || []).map(candidateKey));
      const eligibleKeys = new Set((lastAgentRun?.eligible || []).map(candidateKey));
      const excludedKeys = new Set((lastAgentRun?.excluded || []).map(item => candidateKey(item.candidate)));
      const key = candidateKey(row);
      const statusQuestion = !isSaved
        ? 'Add this location to the shortlist builder'
        : excludedKeys.has(key)
          ? `Why was ${row.name} excluded?`
          : selectedKeys.has(key)
            ? `Why was ${row.name} shortlisted?`
            : eligibleKeys.has(key)
              ? `Why is ${row.name} eligible but not shortlisted?`
              : `Explain ${row.name}'s shortlist status`;
      const evidenceQuestion = flags.length
        ? `What are ${row.name}'s ${flags.length} caution signal${flags.length === 1 ? '' : 's'}?`
        : `Why does ${row.name} have no caution signals?`;
      const questions = [
        statusQuestion,
        evidenceQuestion,
        savedCandidates.length >= 2 ? 'Compare the top two candidates' : 'How reliable is this prediction?',
        brief.evidencePolicy === 'strict' ? 'What changes under a balanced evidence policy?' : 'What changes under a strict evidence policy?',
        'What should I check during field validation?'
      ];
    ...
    function answerCautions(sections, row) {
      const flags = reliabilityFlagDetails(row);
      const evidence = flags.length
        ? flags.map(item => `${item.label.charAt(0).toUpperCase() + item.label.slice(1)}: ${item.detail}`)
        : ['Comparable-anchor error, historical variability, training-range coverage, and joint feature similarity are all within their caution thresholds.'];
      addSection(
        sections,
        'Caution signals',
        flags.length
          ? `${row.name} has ${flags.length} caution signal${flags.length === 1 ? '' : 's'}.`
          : `${row.name} has no caution signals under the current four evidence checks.`,
        evidence,
        flags.length
          ? 'Use the named issues to focus field validation; a caution is diagnostic evidence, not an automatic rejection under every policy.'
          : 'Still validate pedestrian direction, visibility, and local operating conditions before booking media.'
      );
    }
    function answerFieldValidation(sections, row) {
      const flags = reliabilityFlags(row);
      const checks = [
        'Observe pedestrian direction on both sides of the street and whether the proposed display faces the dominant flow.',
        'Check sight lines, viewing distance, obstructions, entrances, crossings, and likely dwell points.',
        'Take short manual counts in at least two relevant time windows and compare the order of magnitude with the weekly estimate.'
      ];
      if (flags.includes('historical variability')) checks.push('Repeat the check on more than one day because comparable anchors show elevated week-to-week variability.');
      if (flags.includes('inputs outside the training range') || flags.includes('weak joint feature similarity')) checks.push('Document the local context carefully because this feature combination has limited support in the training anchors.');
      addSection(
        sections,
        'Field validation',
        `Validate ${row.name} before treating the shortlist status as a media-buying decision.`,
        checks,
        'Record the observation time, weather, route direction, and count method so candidates can be compared consistently.'
      );
    }
    ...
    function composeLocationAnswer(question, row = current) {
      ...
      const asksCautions = /caution|warning|risk flag|evidence issue/.test(q);
      const asksFieldValidation = /field validation|field check|site visit|check on site|validate on site/.test(q);
      const asksDrivers = !asksCautions && /driver|factor|model signal|contribution|explain (?:the )?estimate|why (?:is |does )?(?:this|the).*?(?:high|low|estimate|prediction)/.test(q);
      const asksReliability = !asksFieldValidation && /reliab|confidence|uncertain|accur|precision|error|trust|local evidence|validate/.test(q);
      ...
      if (asksCautions) answerCautions(sections, row);
      ...
      if (asksFieldValidation) answerFieldValidation(sections, row);
    ...
    function answerQuestion() {
      ...
      const answer = composeLocationAnswer(question, current);
      document.getElementById('answer').textContent = answer;
      return answer;
    }
    function parseRequestedBrief(question) {
      const q = question.toLowerCase();
      const requested = {};
      if (/touris|visitor|attraction/.test(q)) requested.objective = 'tourism';
      else if (/retail|shop|high.street/.test(q)) requested.objective = 'retail';
      else if (/transit|commut|station/.test(q)) requested.objective = 'transit';
      else if (/maximum(?: weekly)? exposure|maximize exposure|exposure-led/.test(q)) requested.objective = 'exposure';
      if (/strict/.test(q)) requested.evidencePolicy = 'strict';
      else if (/explor/.test(q)) requested.evidencePolicy = 'exploratory';
      else if (/balanced/.test(q)) requested.evidencePolicy = 'balanced';
      const numberWords = {one: 1, two: 2, three: 3, four: 4, five: 5, six: 6, seven: 7, eight: 8};
      const numericMatch = q.match(/(?:top|shortlist(?: of)?|select|choose|build(?: a)?)\s+(?:a\s+)?([1-8])(?:\s|-|$)/)
        || q.match(/\b([1-8])(?:-site| site)/);
      if (numericMatch) requested.shortlistSize = Number(numericMatch[1]);
      else {
        for (const [word, value] of Object.entries(numberWords)) {
          if (new RegExp(`(?:top|shortlist(?: of)?|select|choose)\\s+${word}|\\b${word}[ -]site`).test(q)) {
            requested.shortlistSize = value;
            break;
          }
        }
      }
    ...
    function applyRequestedBrief(requested) {
      const changes = [];
      if (requested.objective && requested.objective !== document.getElementById('campaignObjective').value) {
        document.getElementById('campaignObjective').value = requested.objective;
        changes.push(`objective=${requested.objective}`);
      }
      if (requested.evidencePolicy && requested.evidencePolicy !== document.getElementById('evidencePolicy').value) {
        document.getElementById('evidencePolicy').value = requested.evidencePolicy;
        changes.push(`evidence policy=${requested.evidencePolicy}`);
      }
      if (requested.shortlistSize && requested.shortlistSize !== Number(document.getElementById('shortlistSize').value)) {
        document.getElementById('shortlistSize').value = requested.shortlistSize;
        changes.push(`shortlist size=${requested.shortlistSize}`);
      }
      renderCampaignRule();
      renderCandidateWorkspace();
      if (current) renderSuggestions(current);
      return changes.length ? `Updated the campaign brief: ${changes.join(', ')}.` : 'Used the campaign brief already shown in the dashboard.';
    }
    function resolveCandidateReference(question) {
      const q = String(question || '').toLowerCase();
      const candidates = [...savedCandidates];
      if (current && !candidates.some(item => candidateKey(item) === candidateKey(current))) candidates.push(current);
      const matches = candidates
        .filter(item => q.includes(String(item.name || '').toLowerCase()))
        .sort((left, right) => String(right.name).length - String(left.name).length);
      if (matches.length) return {candidate: matches[0]};
      const explicitCandidate = String(question || '').match(/(?:map-selected\s+)?candidate\s*\d+/i);
      if (explicitCandidate) return {error: `I could not find ${explicitCandidate[0]} in the candidate table. Select or add that location, then ask again.`};
      return current ? {candidate: current} : {error: 'No location is currently available to explain.'};
    }
    function explainCandidateStatus(question) {
      const resolved = resolveCandidateReference(question);
      if (resolved.error) return resolved.error;
      const row = resolved.candidate;
      const key = candidateKey(row);
      if (!savedCandidates.some(item => candidateKey(item) === key)) {
        return `${row.name} is not in the candidate table. Add it before asking about shortlist status.`;
      }
      if (!lastAgentRun) lastAgentRun = buildShortlist(campaignBrief());
      const selectedIndex = lastAgentRun.selected.findIndex(item => candidateKey(item) === key);
      if (selectedIndex >= 0) {
        return `${row.name} is Shortlisted because it passed the current campaign criteria and ranks ${selectedIndex + 1} among the eligible sites by predicted weekly exposure. It occupies one of the maximum ${lastAgentRun.brief.shortlistSize} shortlist places. Shortlisted means priority for field validation, not a final media-buying recommendation.`;
      }
      const eligibleIndex = lastAgentRun.ranked.findIndex(item => candidateKey(item) === key);
      if (eligibleIndex >= 0) {
        return `${row.name} is Eligible because it passed the current campaign criteria, but it ranks ${eligibleIndex + 1} among eligible sites and falls outside the maximum ${lastAgentRun.brief.shortlistSize} shortlist places.`;
      }
      const excluded = lastAgentRun.excluded.find(item => candidateKey(item.candidate) === key);
      if (excluded) {
        return `${row.name} is Excluded under the current ${lastAgentRun.brief.evidencePolicy} policy because ${excluded.reasons.join('; ')}. Excluded means it does not pass the current screening rules, not that the physical location is inherently poor.`;
      }
      return `${row.name} has no status under the current candidate table.`;
    }
    function planAgentRequest(question) {
      const q = question.trim().toLowerCase();
      const steps = [];
      const requestedBrief = parseRequestedBrief(q);
      if (/load.*sample|sample candidate|example candidate/.test(q)) steps.push({tool: 'load_sample_candidates'});
      if (/save|add (?:this|current)|remember/.test(q) && !/saved candidate/.test(q)) steps.push({tool: 'save_current_candidate'});
      if (/clear|remove all/.test(q) && /candidate|workspace/.test(q)) steps.push({tool: 'clear_candidate_workspace'});
      const asksSingleShortlist = /should.*this.*shortlist|this location.*shortlist|shortlist.*this location/.test(q);
      const asksCandidateStatus = /why (?:was|is).*?(?:shortlisted|eligible|excluded)|eligible but not shortlisted|shortlist status/.test(q);
      if (/compare|difference|best of (?:the )?saved/.test(q)) steps.push({tool: 'compare_saved_candidates'});
      if (/what.*change|switch.*policy|under (?:a )?(?:strict|balanced|exploratory)/.test(q) && Object.keys(requestedBrief).length) {
        steps.push({tool: 'set_campaign_brief', args: requestedBrief});
        steps.push({tool: 'build_shortlist'});
      }
      if (!asksSingleShortlist && !asksCandidateStatus && /shortlist|rank|top candidate|best candidate/.test(q)) {
        if (!steps.some(step => step.tool === 'set_campaign_brief')) steps.push({tool: 'set_campaign_brief', args: requestedBrief});
        if (!steps.some(step => step.tool === 'build_shortlist')) steps.push({tool: 'build_shortlist'});
      }
      if (asksCandidateStatus) {
        steps.push({tool: 'explain_candidate_status', args: {question}});
      } else if (/why.*(?:excluded|rejected|not selected)|explain.*exclusion|candidates excluded/.test(q)) {
        if (!steps.some(step => step.tool === 'build_shortlist') && !lastAgentRun) steps.push({tool: 'build_shortlist'});
        steps.push({tool: 'explain_exclusions', args: {question}});
      }
      const asksLocationEvidence = asksSingleShortlist || /reliab|confidence|uncertain|error|driver|factor|signal|caution|warning|contribution|touris|retail|horeca|transit|anchor|distance|training range|outside.*range|weekly passers|footfall estimate|field validation|site visit|what can you/.test(q);
      if (asksLocationEvidence && !steps.some(step => step.tool === 'build_shortlist')) {
        steps.push({tool: 'explain_current_location', args: {question}});
      }
      if (!steps.length) steps.push({tool: 'show_agent_scope'});
      return steps;
    }
    function campaignFitText(row, objective) {
      if (objective === 'tourism') return `Tourism p${Math.round(row.tourismPct)}`;
      if (objective === 'retail') return `Retail p${Math.round(row.retailPct)}`;
      if (objective === 'transit') return `Transit p${Math.round(row.transitPct)}`;
      return 'Exposure-led';
    }
    function executeAgentTool(step) {
      if (step.tool === 'save_current_candidate') {
        return {trace: ['save_current_candidate'], message: saveCurrentCandidate()};
      }
      if (step.tool === 'load_sample_candidates') {
        return {trace: ['evaluate_sample_locations', 'save_candidates'], message: loadSampleCandidates()};
      }
      if (step.tool === 'clear_candidate_workspace') {
        savedCandidates = [];
        lastAgentRun = null;
        persistCandidates();
        renderCandidateWorkspace();
        return {trace: ['clear_candidate_workspace'], message: 'Cleared the candidate workspace.'};
      }
      if (step.tool === 'set_campaign_brief') {
        return {trace: ['parse_campaign_brief', 'set_campaign_brief'], message: applyRequestedBrief(step.args || {})};
      }
      if (step.tool === 'compare_saved_candidates') {
        if (savedCandidates.length < 2) {
          return {trace: ['compare_saved_candidates'], message: 'Comparison needs at least two candidate locations. Add another location or try the demo set.'};
        }
        const ranked = rankCandidates(savedCandidates);
        const leader = ranked[0];
        const runnerUp = ranked[1];
        const leadPct = runnerUp?.predicted ? Math.round(100 * (leader.predicted - runnerUp.predicted) / runnerUp.predicted) : 0;
        return {
          trace: ['read_candidate_workspace', 'compare_saved_candidates'],
          message: `The shortlist table is already ranked by predicted weekly exposure. ${leader.name} leads with ${fmt(leader.predicted)} weekly passers, ${leadPct}% above ${runnerUp.name} at ${fmt(runnerUp.predicted)}. ${leader.name} has ${evidenceSummary(leader)}; ${runnerUp.name} has ${evidenceSummary(runnerUp)}. Use the intervals and status reasons in the table before treating this as a final fieldwork priority.`
        };
      }
      if (step.tool === 'build_shortlist') {
        if (!savedCandidates.length) {
          return {trace: ['read_candidate_workspace'], message: 'The candidate workspace is empty. Save candidate locations or load samples before building a shortlist.'};
        }
        lastAgentRun = buildShortlist(campaignBrief());
        renderCandidateWorkspace();
        if (!lastAgentRun.selected.length) {
          return {
            trace: ['read_campaign_brief', 'filter_candidates', 'rank_candidates', 'generate_shortlist'],
            message: `No saved candidate passed the ${lastAgentRun.brief.evidencePolicy} evidence and ${objectiveLabels[lastAgentRun.brief.objective]} rules. ${lastAgentRun.excluded.length} candidate${lastAgentRun.excluded.length === 1 ? ' was' : 's were'} excluded; ask why they were excluded before relaxing the policy.`
          };
        }
        const names = lastAgentRun.selected.map((row, index) => `${index + 1}. ${row.name} (${fmt(row.predicted)})`).join('; ');
        return {
          trace: ['read_campaign_brief', 'filter_candidates', 'rank_candidates', 'generate_shortlist'],
          message: `Generated a ${lastAgentRun.brief.evidencePolicy} ${objectiveLabels[lastAgentRun.brief.objective]} shortlist. ${lastAgentRun.eligible.length} of ${savedCandidates.length} candidates were eligible. Selected: ${names}.`
        };
      }
      if (step.tool === 'explain_candidate_status') {
        return {trace: ['read_campaign_brief', 'read_candidate_status'], message: explainCandidateStatus(step.args?.question || '')};
      }
      if (step.tool === 'explain_exclusions') {
        if (!lastAgentRun) return {trace: ['explain_exclusions'], message: 'Build a shortlist first so exclusions can be tied to a specific campaign brief.'};
        if (!lastAgentRun.excluded.length) return {trace: ['explain_exclusions'], message: 'No candidates were excluded under the current campaign brief.'};
        const q = String(step.args?.question || '').toLowerCase();
        let matches = lastAgentRun.excluded;
        if (/this location|current location|this site/.test(q) && current) {
          matches = matches.filter(item => candidateKey(item.candidate) === candidateKey(current));
          if (!matches.length) {
            const isSelected = lastAgentRun.selected.some(item => candidateKey(item) === candidateKey(current));
            const isEligible = lastAgentRun.eligible.some(item => candidateKey(item) === candidateKey(current));
            if (isSelected) return {trace: ['read_current_status'], message: `${current.name} was not excluded. It is currently shortlisted for field validation.`};
            if (isEligible) return {trace: ['read_current_status'], message: `${current.name} passed the criteria but falls below the current shortlist cutoff.`};
            return {trace: ['read_current_status'], message: `${current.name} is not in the candidate table. Add it before asking about shortlist status.`};
          }
        } else {
          const named = matches.filter(item => q.includes(item.candidate.name.toLowerCase()));
          if (named.length) matches = named;
        }
        const details = matches.map(item => `${item.candidate.name}: ${item.reasons.join('; ')}`).join('\n- ');
        return {trace: ['read_last_shortlist', 'explain_exclusions'], message: `Excluded candidates:\n- ${details}`};
      }
      if (step.tool === 'explain_current_location') {
        if (!current) runEvaluation();
        const resolved = resolveCandidateReference(step.args?.question || '');
        if (resolved.error) return {trace: ['resolve_candidate_reference'], message: resolved.error};
        return {trace: ['resolve_candidate_reference', 'explain_model_evidence'], message: composeLocationAnswer(step.args?.question || '', resolved.candidate)};
      }
      return {
        trace: ['show_agent_scope'],
        message: 'I can save the current location, load examples, compare saved candidates, build a campaign shortlist, explain exclusions, and answer evidence questions about the selected location. I cannot access inventory price, availability, demographics, screen visibility, or campaign ROI.'
      };
    }
    function runAgent() {
      if (!current) runEvaluation();
      const question = document.getElementById('askInput').value.trim();
      const plan = planAgentRequest(question);
      const trace = ['plan_request'];
      const messages = [];
      for (const step of plan) {
        const result = executeAgentTool(step);
        trace.push(...result.trace);
        if (result.message) messages.push(result.message);
      }
      document.getElementById('answer').textContent = messages.join('\n\n');
      if (current) renderSuggestions(current);
      return {plan, trace, messages};
    }
    ...
    document.getElementById('saveCandidateBtn').addEventListener('click', () => {
      document.getElementById('answer').textContent = saveCurrentCandidate();
      renderSuggestions(current);
    });
    document.getElementById('sampleCandidatesBtn').addEventListener('click', () => {
      document.getElementById('answer').textContent = loadSampleCandidates();
      renderSuggestions(current);
    });
    document.getElementById('clearCandidatesBtn').addEventListener('click', () => {
      ...
      renderCandidateWorkspace();
      renderSuggestions(current);
      document.getElementById('answer').textContent = 'Cleared all candidate locations.';
    });
    ['campaignObjective', 'evidencePolicy'].forEach(id => {
    ...
    document.getElementById('shortlistSize').addEventListener('input', () => {
      lastAgentRun = null;
      renderCandidateWorkspace();
      if (current) renderSuggestions(current);
    });
    document.getElementById('askBtn').addEventListener('click', runAgent);
...
"""
    ...
    (APP_DIR / "dashboard_agent.html").write_text(html, encoding="utf-8")
    print("Generated app/dashboard_agent.html")
